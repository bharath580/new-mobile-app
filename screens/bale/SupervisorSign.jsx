...
import {addSupervisorSign, balePostData} from '../../features/bale/baleSlice';

const SupervisorSign = ({navigation}) => {
  ...
  const handleOK = async base64DataUrl => {
    try {
      ...
      dispatch(addSupervisorSign(base64DataUrl)); // Dispatch with the provided data
      console.log('action');
    } catch (error) {
  ...
  const handleNext = async () => {
    try {
      ...
      await dispatch(balePostData(orderData)).unwrap();
      navigation.navigate('BaleList');
    } catch (e) {
      console.log(e);
    }
  };
